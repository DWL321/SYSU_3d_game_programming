...
namespace mygame
{
    ...
    public class Move : MonoBehaviour
    {
        ...
        int move_type = 0;                        //0是不动，1水平移动，2竖直移动
        Vector3 to_pos;
        Vector3 middle_pos;
        ...
        void Update()
        {
            ...
            {
                transform.position = Vector3.MoveTowards(transform.position, to_pos, move_speed * Time.deltaTime);
                if (transform.position == to_pos)
                    move_type = 0;           
            }
        }
        ...
        {
            to_pos = position;
            if(position == transform.position) return ;      //没有移动直接返回
            else if (position.y == transform.position.y) move_type = 2;        //船水平移动
            else if (position.y < transform.position.y)      //角色从陆地到船，先竖直移动再水平移动
            {
                middle_pos = new Vector3(position.x, transform.position.y, position.z);
                move_type = 1;
            }
            else                                          //角色从船到陆地，先竖直移动再水平移动
            {
